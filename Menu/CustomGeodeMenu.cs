...
{
    internal class GeodeMenu : MenuWithInventory
    {
        // =============== constants ===============
        ...
        private bool _autoOpenAmountActive;
        private bool _autoUntilFull;
        ...
        // =============== logging ===============
        private void Err(string m) { try { _monitor.Log($"[GeodeMenu] {m}", LogLevel.Error); } catch { } }
        ...
        public GeodeMenu(
            ...
            Dictionary<string, Texture2D?> smithingTextures,
            Action onClosed)
            : base(null, okButton: true, trashCan: true, 12, 132)
        {
            ...
            smithingTextures.TryGetValue("auto_buttons",  out _texAutoButtons);
            smithingTextures.TryGetValue("smash_buttons", out _texSmashButtons);
            smithingTextures.TryGetValue("hammer",        out _texHammer);
            ...
            _openXAmount = Math.Max(1, config.AmountGeodesToOpen);
        ...
        {
            try { var p = cfg.GetType().GetProperty(name); if (p?.GetValue(cfg) is bool b) return b; } catch { }
            return def;
        }
        ...
        private bool HighlightGeodes(Item i)
        {
            if (heldItem == null)
            {
                try { return Utility.IsGeode(i) && _config.GeodeAllowances[ToolType.Geode][i.ItemId]; }
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = true)
        {
            ...
            if (_btnOpenX.containsPoint(x, y))
            {
                ...
                if (have <= 0)
                {
                    descriptionText = _translation.Get("geode.empty");
        ...
        private void UpdateGeodeReveal(GameTime time)
        {
            ...
            if (finished)
            {
                if (_sparkle == null)
                {
                    if (!(_geodeTreasure is Object obj) || obj.Price > 75 ||
        ...
        private void AdvanceIdle(GameTime time)
        {
            ...
            while (_idleTimerMs >= _idleFrameIntervalMs)
            {
                ...
                int baseOffset = _idleOffsets[_idleFrameIdx];
        ...
        private void DrawHammer(SpriteBatch batch, Vector2 farmerOriginScreen, bool flip)
        {
            try
            {
                ...
                var src = new Rectangle(0, 0, _texHammer.Width, _texHammer.Height);
        ...
        // =============== helpers ===============
        private string LeftButtonText()  => _translation.Get("geode.smash-x-button", new { amount = _openXAmount });
        private string RightButtonText() => _translation.Get("geode.auto-smash-button");
        ...
        private bool CanAcceptOneResult() => Game1.player.freeSpotsInInventory() >= 1;
